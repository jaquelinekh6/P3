...
namespace upc {
  void PitchAnalyzer::autocorrelation(const vector<float> &x, vector<float> &r) const {
    ...
    /**
      ...
      \f]
      - Inicializamos la atuocorrelación a 0.
      - Sumamos la multipliacación de las dos (la señal natural con la desplazada)
      */
  ...
  float PitchAnalyzer::compute_pitch(vector<float> & x) const {
    ...
    vector<float>::const_iterator iR = r.begin(), iRMax = iR + npitch_min; //OJO!
    ...
    iRMax = std::max_element(iR + npitch_min, iR + npitch_max); //devuelve el iterador que apunta la posicion del maximo
